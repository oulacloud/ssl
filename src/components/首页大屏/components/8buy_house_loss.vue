      ...
      await axios.get("http://sslapi.nadibao.cn/api/HzYaoHaoData/Get_PEOPLE_PROVINCE_CITY_NUM_ALLYEAR")
        .then(res => {
          ...
          for (var i = 0; i < this.resData.length; i++) {
            if (this.resData[i].NAME == 'FAMILY_BUYER') {
              this.x[i] = this.resData[i].NAME
              this.y1[i] = parseFloat((this.resData[i].NUM / 10000).toFixed(2))
              this.rate[i] = parseFloat((this.resData[i].RATE).toFixed(2))
              this.data.push({ value: parseFloat((this.resData[i].RATE * 100).toFixed(2)), name: '家庭购房', itemStyle: { color: this.color[i], }, })
            }
            else if (this.resData[i].NAME == 'PERSONAL_BUYER') {
              this.x[i] = this.resData[i].NAME
              this.y1[i] = parseFloat((this.resData[i].NUM / 10000).toFixed(2))
              this.rate[i] = parseFloat((this.resData[i].RATE).toFixed(2))
              this.data.push({ value: parseFloat((this.resData[i].RATE * 100).toFixed(2)), name: '个人购房', itemStyle: { color: this.color[i], }, })
            }
          }
          // console.log(this.y)
        })
