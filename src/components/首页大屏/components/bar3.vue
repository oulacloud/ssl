      ...
      await axios.get("http://sslapi.nadibao.cn/api/HzYaoHaoData/Get_MID_TALENT_BUYER_TYPE_NUM_ALLYEAR")
        .then(res => {
          ...
          for (var i = 0; i < this.resData.length; i++) {
            this.name[i] = this.resData[i].YEAR_NAME
            this.zz[i] = this.resData[i].TYPE_ZHONGZHUAN_NUM
            this.dz[i] = this.resData[i].TYPE_DAZHUAN_NUM
            this.bk[i] = this.resData[i].TYPE_BENKE_NUM
            this.ss[i] = this.resData[i].TYPE_SHUOSHI_NUM
            this.bs[i] = this.resData[i].TYPE_BOSHI_NUM
            this.cj[i] = this.resData[i].TYPE_CHUJI_NUM
            this.zj[i] = this.resData[i].TYPE_ZHONGJI_NUM
            this.gj[i] = this.resData[i].TYPE_GAOJI_NUM
            this.qt[i] = this.resData[i].TYPE_ELSE_NUM
            this.non[i] = this.resData[i].TYPE_NONE_NUM
    ...
    drawChart() {
      ...
      var option = {
        ...
        series: [{
          ...
          barWidth: '30%',
          label: {
            show: this.dialogVisable ? this.label_show : false,
            position: 'top',
            color: '#000',
            formatter: '{c}'
          },
