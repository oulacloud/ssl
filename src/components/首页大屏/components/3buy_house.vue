      ...
      await axios.get("http://sslapi.nadibao.cn/api/HzYaoHaoData/Get_MID_YEAR_ADD_NUM_ONEYEAR_AND_ALLYEAAR")
        .then(res => {
          ...
          for (var i = 0; i < this.resData.length; i++) {
            ...
            this.y1[i] = parseFloat((this.resData[i].NUM_RENCI / 10000).toFixed(2))
            this.y2[i] = parseFloat((this.resData[i].NUM_RENSHU / 10000).toFixed(2))
            this.rate[i] = parseFloat((this.resData[i].RENCI_DIVISION_RENSHU).toFixed(2))
            // this.yearData.push({ value: this.resData[i].RENCI, name: this.resData[i].YEAR })
          }
          // console.log(this.y)
        })
    },
