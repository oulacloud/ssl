<template>
  <div class="LVbody">
    ...
    <section class="mainbox">
      ...
      <div class="column">
        <template>
          <el-table :data="topData" style="width: 100%" v-if="flag">
            <el-table-column label="摇号登记次数top10人员">
              <el-table-column prop="ROWID" label="排序" width="90%" style="text-align: center">
              </el-table-column>
              <el-table-column prop="NAME" label="唯一识别" width="180%">
              </el-table-column>
              <el-table-column prop="NUM" label="摇号次数" width="90%">
              </el-table-column>
              <!-- <el-table-column prop="girl" label="女性户数" width="80%">
              </el-table-column> -->
            </el-table-column>
          </el-table>
        </template>
...
export default {
  ...
  methods: {
    ...
    async getData() {
      await axios.get("http://sslapi.nadibao.cn/api/HzYaoHaoData/GetHZ_YAOHAO_ONEPEOPLE_REG_TIME")
        .then(res => {
          this.data = res.data.data
          for (var i = 0; i < res.data.data.length; i++) {
            this.x[i] = res.data.data[i].NAME
            this.y1[i] = parseFloat((res.data.data[i].NUM).toFixed(2))
            // this.y2[i] = parseFloat((res.data.data[i].NUM2 / 10000).toFixed(2))
            this.sum += this.y1[i]
            this.rate[i] = parseFloat((res.data.data[i].RATE * 100).toFixed(2))
            // this.yearData.push({ value: res.data.data[i].RENCI, name: res.data.data[i].YEAR })
          }
          // console.log(this.y)
        })
    },
    async getData1() {
      await axios.get("http://sslapi.nadibao.cn/api/HzYaoHaoData/GetHZ_YAOHAO_ONEPEOPLE_TOP10_TABLE")
        .then(res => {
    ...
    drawChart() {
      ...
      var option = {
        ...
        },
        {
          ...
          data: [...this.y1].map(x => (x * 100 / this.sum).toFixed(2))
        }]
