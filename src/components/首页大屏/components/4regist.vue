      ...
      await axios.get("http://sslapi.nadibao.cn/api/HzYaoHaoData/GetHZ_YAOHAO_ONEPEOPLE_REG_TIME")
        .then(res => {
          ...
          for (var i = 0; i < this.resData.length; i++) {
            ...
            this.y1[i] = parseFloat((this.resData[i].NUM).toFixed(2))
            // this.y2[i] = parseFloat((this.resData[i].NUM2 / 10000).toFixed(2))
            this.sum += this.y1[i]
            this.rate[i] = parseFloat((this.resData[i].RATE * 100).toFixed(2))
            // this.yearData.push({ value: this.data[i].RENCI, name: this.data[i].YEAR })
          }
          // console.log(this.y)
        })
    },
