...
export default {
  ...
  data() {
    return {
      ...
      le_show: this.dialogVisable,
      h_v: this.dialogVisable,
      tool_show: this.dialogVisable,
      label_show: this.dialogVisable,
      graphic_show: !this.dialogVisable,
      top: 5,
      left: 42,
      backgroundColor: '#fff',
      titleColor: '#000000',
      ...
      title_left: 50,
      fontSize: 16,
      x_show: this.dialogVisable,
      y_show: this.dialogVisable,
      y2_show: this.dialogVisable,
      x_unit: '年份',
      y_unit: '人次',
      y2_unit: '同比增长',
      imageUrl: '',
  ...
  methods: {
    zoom() {
      ...
      this.$nextTick(() => {
        ...
        vm.drawChart()
        if (chartFull) {
          this.fullChart = echarts.init(document.querySelector('.mycharts'));
          let options = this.myChart().option;
        ...
        toolbox: {
          ...
          feature: {
            myTool: {
              ...
              onclick: () => {
                // 全屏查看
                vm.all_show = true// 打开弹窗
                vm.dialogVisable = true
                console.log(vm.all_show)
                // vm.drawChart()
                this.$nextTick(() => {
                const chartFul = echarts.init(this.$refs.fullOne, this.currentTheme)

                console.log(chartFul)
                if (chartFul) {
                  vm.fullChart = echarts.init(this.$refs.fullOne, this.currentTheme)// 画图
                  // vm.fullChart.setOption(option)
                  // vm.drawChart()
                }
                })
              }
