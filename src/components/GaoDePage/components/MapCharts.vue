<template>
    <el-card id="echartsCard">
        <div id="map" class="map">
            ...
            <el-drawer :visible.sync="drawer" direction="ltr" size="15%" :modal="false" custon-class="drawer" title="修改配置项">
                <el-tabs type="border-card">

                    <!-- 文本选项 -->
                    <el-tab-pane>
                        <span slot="label"><i class="el-icon-setting"></i>文本选项</span>

                        <!-- 标题文本控制 -->
                        <el-collapse accordion>
                            <el-collapse-item title="标题文本" name="1">
                        ...
                        <!-- 地图文本控制 -->
                        <el-collapse accordion>
                            <el-collapse-item title="地图文本" name="2">
                    ...
                    <el-tab-pane>
                        ...
                        <div @mouseover="mouseOver(index)" @mouseleave="mouseLeave()" class="col-body"
                            v-for="(item, index) in colors_list" :key="index" @click=change_colors(item)>
...
export default {
    ...
    methods: {
        ...
        get_polygon() {
            ...
            axios.get('/static/gaode_json/Province_Polygon.json')
                .then(res => {
        ...
        get_line() {
            const cancelToken = axios.CancelToken;
            const source = cancelToken.source();
            axios.get('/static/gaode_json/Province_Line.geojson')
                .then(res => {
        ...
        set_hex_color(feature) {
            ...
            if (feature.coordinates[0].properties.PROVINCE_NAME == '浙江省') {
                color = this.colors[data_range_len + 1]
            }
            else {
                if (value >= this.data_range[data_range_len - 1]) {
                    color = this.colors[data_range_len]
    ...
    async mounted() {
        this.colors = this.colors_list[0].colors
        this.current_value = this.colors_list[0].value
