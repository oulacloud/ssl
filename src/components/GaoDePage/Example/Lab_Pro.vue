...
export default {
    data() {
        return {
            ...
            map_colors: null,
            map_font_size: [12, 14, 16, 18, 20, 22],
    ...
    methods: {
        ...
        json_geojson() {
            for (var i = 0; i < this.data_json.length; i++) {
                ...
                list.coord = this.data_json[i].LON_LAT
                list.value = this.data_json[i].NUM
                list.name = this.data_json[i].PROVINCE_NAME
            ...
            this.LabelsLayer.setOptions({
                style: {
                    ...
                    text: function (data) {
                        ...
                        value = (value * 100 / that.sum_num).toFixed(2)
                        return name + value + '%'
                    },
                    ...
                    fontSize: function (data) {
                        var size = that.set_lab_text(data)
                        return size
                    },
                    strokeColor: '#000',
                    strokeWidth: 0,
        ...
        set_lab_text(data) {
            ...
            var value = data.value.value * 100 / this.sum_num
            if (value >= this.data_range[data_range_len - 1]) {
                size = this.map_font_size[data_range_len]
            }
            else {
                for (var i = 0; i < data_range_len; i++) {
                    if (value < this.data_range[i]) {
                        size = this.map_font_size[i]
                        break;
                    }
                }
            }
