...
export default {
  ...
  data() {
    return {
      ...
      // 数据
      name: ['老杭州人', '省内新杭州人', '省外新杭州人', '老杭州男性', '老杭州女性', '省内新杭州男性', '省内新杭州女性', '省外新杭州男性', '省外新杭州女性'],
      list1: [],
      list2: [],
      nums: [],
      tableData: [],
      ...
      await axios.get("http://sslapi.nadibao.cn/api/HzYaoHaoData/Get_PEOPLE_PROVINCE_CITY_NUM_ALLYEAR")
        .then(res => {
          ...
          for (var i = 0; i < this.resData.length; i++) {
            // 老杭州人
            if (this.resData[i].NAME == 'CITY_LOCAL') {
              var list = {}
              list.name = this.name[0]
              list.value = parseFloat((this.resData[i].RATE * 100).toFixed(2))
              this.list1[0] = list
            }
            // 省内新杭州人
            else if (this.resData[i].NAME == 'OUTCITY_INPROVINCE') {
              var list = {}
            ...
            // 省外新杭州人
            else if (this.resData[i].NAME == 'OUTCITY_OUTPROVINCE') {
              var list = {}
            ...
            // 老杭州男性
            else if (this.resData[i].NAME == 'CITY_LOCAL_MALE') {
              var list = {}
              this.nums[0] = this.resData[i]
              list.name = this.name[3]
              list.value = parseFloat((this.resData[i].RATE * 100).toFixed(2))
              this.list2[0] = list
            }
            // 老杭州女性
            else if (this.resData[i].NAME == 'CITY_LOCAL_FEMALE') {
              var list = {}
              this.nums[1] = this.resData[i]
              list.name = this.name[4]
              list.value = parseFloat((this.resData[i].RATE * 100).toFixed(2))
              this.list2[1] = list
            }
            // 省内新杭州男性
            else if (this.resData[i].NAME == 'OUTCITY_INPROVINCE_MALE') {
              var list = {}
              this.nums[2] = this.resData[i]
              list.name = this.name[5]
              list.value = parseFloat((this.resData[i].RATE * 100).toFixed(2))
              this.list2[2] = list
            }
            // 省内新杭州女性
            else if (this.resData[i].NAME == 'OUTCITY_INPROVINCE_FEMALE') {
              var list = {}
              this.nums[3] = this.resData[i]
              list.name = this.name[6]
              list.value = parseFloat((this.resData[i].RATE * 100).toFixed(2))
              this.list2[3] = list
            }
            // 省外新杭州男性
            else if (this.resData[i].NAME == 'OUTCITY_OUTPROVINCE_MALE') {
              var list = {}
              this.nums[4] = this.resData[i]
              list.name = this.name[7]
              list.value = parseFloat((this.resData[i].RATE * 100).toFixed(2))
              this.list2[4] = list
            }
            // 省外新杭州女性
            else if (this.resData[i].NAME == 'OUTCITY_OUTPROVINCE_FEMALE') {
              var list = {}
              this.nums[5] = this.resData[i]
        ...
        series: [
          {
            ...
            radius: this.dialogVisable ? [0, 120] : [0, 50],
            itemStyle: {
              borderWidth: 4,
              borderColor: '#FFFFFF',
            },
          ...
          },
          {
            ...
            radius: this.dialogVisable ? [122, 180] : [51, 80],
            itemStyle: {
              borderWidth: 4,
              borderColor: '#FFFFFF',
            },
