      ...
      await axios.get("http://sslapi.nadibao.cn/api/HzYaoHaoData/Get_MID_YEAR_ADD_NUM_ONEYEAR_AND_ALLYEAAR")
        .then(res => {
          for (var i = 0; i < res.data.data.length; i++) {
            ...
            this.nd[i] = (res.data.data[i].NUM_ONEYEAR / 10000).toFixed(2)
            this.lj[i] = (res.data.data[i].NUM_ALLYEAR / 10000).toFixed(2)
            this.cz[i] = (res.data.data[i].NUM_ONE_CUT_ALL).toFixed(0)
          ...
          },
          {
            ...
            lineStyle: {
              color: new echarts.graphic.LinearGradient(0, 0, 0, 1, [
                {
                  offset: 0,
                  color: 'rgba(255, 227, 168, .8)',
                },
                {
                  offset: 0.5,
                  color: 'rgba(255, 227, 168, .5)',
                },
                {
                  offset: 1,
