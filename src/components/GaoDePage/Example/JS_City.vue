...
export default {
    ...
    methods: {
        ...
        async get_datajson() {
            ...
            this.data_city = res.data.data;
            for (var i = 0; i < this.data_city.length; i++) {
                if (this.data_city[i].PROVINCE_NAME != '浙江省') {
                    this.sum_num = this.sum_num + this.data_city[i].NUM;
                }
            }
        },
        // 获取面数据
        get_geojson_polygon() {
            ...
            axios.get('/static/gaode_json/City_Pol_GD.json', {
        ...
        set_map_fillColor(geojson, lnglats) {
            ...
            for (var i = 0; i < this.data_city.length; i++) {
                if (this.data_city[i].CITY_NAME == pro_name) {
                    value = this.data_city[i].NUM * 100 / this.sum_num;
                }
            }
            // 值为空，面颜色赋值为默认
            if (value == 0) {
            ...
            if (pro_name == '浙江') {
                var marker = new AMap.Marker({
                    ...
                    icon: require('../GaoDeStatic/marker_1.png'),
                    // offset : (-8,-8),
                    anchor: 'center',
                });
                this.map.add(marker);
